...
<head>
	...
	<title>ACTIVITY 3 BY NARCISO MEDURA</title>
</head>
			...
			?>
				<td>
					<form method="post" enctype="multipart/form-data" action="?edit_id=<?php echo $row['menu_id']; ?>" style="display: inline;">
						...
						<button type="button" class="btn btn-primary" data-toggle="modal" data-target="#editMenuModal" data-menu-id="<?php echo $row['menu_id']; ?>" data-menu-name="<?php echo $row['menu_name']; ?>" data-menu-desc="<?php echo $row['menu_desc']; ?>" data-price="<?php echo $row['price']; ?>">EDIT</button>&nbsp;
					</form>

					<form method="post" style="display: inline;">
						<input type="hidden" name="delete" value="<?php echo $row['menu_id']; ?>">
						<button type="button" class="btn btn-danger" data-toggle="modal" data-target="#deleteMenuModal" data-menu-id="<?php echo $row['menu_id']; ?>">DELETE</button>
					</form>
				</td>
			<?php echo "</tr>";
			}
